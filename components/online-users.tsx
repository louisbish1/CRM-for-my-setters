...
export function OnlineUsers({
  ...
  }, [currentUserId, userEmail, userLabel]);
  ...
  }, [currentUserId, knownUsers, onlineUsers, userLabel]);
        ...
        <div className="absolute right-0 z-20 mt-2 w-72 rounded-2xl border border-white/10 bg-zinc-950/95 p-2 shadow-glow backdrop-blur-xl">
          ...
          <div className="grid gap-1">
            {visibleUsers.map((user) => (
              <div key={user.id} className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-white/75">
                <Circle
                  className={
                    user.online
                      ? "h-2.5 w-2.5 fill-emerald-300 text-emerald-300"
                      : "h-2.5 w-2.5 fill-white/20 text-white/20"
                  }
                />
